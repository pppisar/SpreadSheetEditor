...
CStateTxt::CStateTxt(CTable* table, std::string & fileName, std::string & path) 
: CStateManager(table, fileName, path) {}

bool CStateTxt::save() {
    std::ofstream file(m_path + m_fileName + ".txt", std::ios::trunc);
...
bool CStateTxt::load() {
    std::ifstream file(m_path + m_fileName + ".txt");
